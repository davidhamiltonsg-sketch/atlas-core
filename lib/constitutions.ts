...
export const ATLAS_CORE: Constitution = {
  ...
  docPath: "/downloads/atlas-core-constitution-v10.6.html",
  ...
  decisionLadder: [],
  rules: [
    { category: "Authority", title: "The written constitution controls", description: "Atlas v10.6 governs. The app implements its thresholds and contribution-first decision process; any disagreement must be resolved in favour of the written constitution." },
    ...
    { category: "Contributions", title: "The rebalancing rule, written down", description: "If VWRA breaches 65% or 75% of the portfolio, steer it back toward 70% at the next monthly contribution or the quarterly review, whichever comes first. Drift is repaired with new money; a market fall alone never forces a sale." },
    { category: "Portfolio construction", title: "Bitcoin and semiconductors keep a 2% minimum", description: "BTC and SMH each hold a 2% hard floor as deliberate conviction positions. Below the floor, contributions top the sleeve back up; above the cap, additions pause. Removing either sleeve requires a written amendment, not quiet attrition during a drawdown." },
    { category: "Portfolio construction", title: "The EQAC + SMH overlap is known and offset", description: "EQAC and SMH together create roughly 15% deliberate technology concentration (ceiling 25%). DBMFE is the designed offset during corrections. Both falling together is expected behaviour, not a signal to panic-rebalance." },
...
const SBR_PRESENTATION: Record<string, { name: string; role: string; color: string; note?: string }> = {
  ...
  BTC: { name: "Bitcoin sleeve", role: "A capped asymmetric return sleeve with a 2% minimum conviction floor", color: "#f59e0b" },
...
export const SILICON_BRICK_ROAD: Constitution = {
  ...
  broker: "IBKR Singapore",
  docPath: "/downloads/silicon-brick-road-constitution-v10.5.html",
  funds: SBR_FUNDS,
  ...
  rules: [
    ...
    { category: "Keeping Things in Balance", title: "The rebalancing rule, written down", description: "If VWRA rises above 70% or falls below 60% of the portfolio, direct the following monthly contributions toward VWRA (or away from it) until it is back around its 65% target. Repair drift with new money, never with panic selling." },
    { category: "Keeping Things in Balance", title: "Bitcoin keeps a 2% minimum", description: "Bitcoin holds a 2% floor as a minimum conviction position. If it drifts below 2%, contributions top it back up; above 8%, additions pause. If the conviction ever goes, the right move is a written amendment to remove the sleeve, not quiet neglect." },
    ...
    { category: "Changing the Rules", title: "Bands are re-examined every year", description: "The wide VWRA band (60\u201370%) is a deliberate choice, not an accident. Each annual review re-examines every band against how the portfolio actually behaved; tightening them is a one-line amendment if drift proves troublesome." },
  ],
  scorecard: [
