...
export const ATLAS_CORE: Constitution = {
  ...
  rules: [
    ...
    { category: "Art. XI — Shock Buffer", title: "Buffer deployment trigger — 12% ceiling", description: "If SGOV exceeds 12% of the portfolio at any dealing window, deploy half the excess above 8% into VT in that same window. Example: SGOV at 14% → deploy 3% into VT, bringing SGOV from 14% to 11%. This prevents the shock buffer from becoming a chronic performance drag (the expected cost of excess buffer is roughly 0.4–0.5% CAGR per excess percentage point held). The 8% floor is never violated; only the excess above 12% triggers deployment. Do not sell other positions to build SGOV — grow it only from new money and fund distributions." },
    { category: "Art. IX — Look-Through", title: "Minimum viable monthly look-through check", description: "The full look-through computation (run via the Look-Through page) takes 10–15 minutes quarterly. For the monthly quick-check, use this shortcut: look up the top-3 holdings of each of QQQM, SMH, and VT in your broker's fund overview. If none of those names appear in more than one fund's top-3 simultaneously, single-company concentration is almost certainly fine. Run the full computation once per quarter regardless — the shortcut detects obvious drift; the full run catches accumulated overlap. If you have not run the full look-through in more than 90 days, the Freshness dimension of the governance score will degrade." },
    { category: "Art. XVIII — Sell Criteria", title: "Broken-thesis proxies — observable triggers", description: "Each position has a thesis. These proxies signal when the thesis may be broken: QQQM — watch if NASDAQ-100 companies' aggregate earnings as a share of global-ex-US tech revenue falls below 25% for two consecutive fiscal years (US mega-cap dominance thesis eroding). SMH — watch if global semiconductor capital expenditure contracts for three consecutive quarters without a corresponding price increase (capex cycle thesis inverting). BTC — the thesis breaks if a consensus-critical protocol vulnerability is successfully exploited at scale — not a price drawdown or regulatory news, but an actual protocol failure. VT/VWO — these are structural diversifiers, not theses; they are never sold for thesis reasons, only for rebalancing. If a proxy crosses its threshold, apply the 72-hour rule (Art. XIV) before taking any action. Price action alone is never sufficient." },
  ],
}
...
const SBR_PRESENTATION: Record<string, { name: string; role: string; color: string; note?: string }> = {
  ...
  SMH:  { name: "VanEck Semiconductor ETF",          role: "Growth tilt — semiconductors (most volatile)", color: "#818cf8", note: "Only mandatory sell in the portfolio — trim to 15% if it exceeds its phase cap (Phase I: 20%, Phase II: 18%, Phase III: 16%, Phase IV: 14%). Cap tightens as you near the goal." },
...
const SBR_PHASE_COPY: Array<Omit<ConstitutionPhase, "min" | "max">> = [
  ...
  { key: "II",  label: "Phase II — Controlled growth", range: "SGD 72,000–96,000",  selling: false, body: "No selling. Redirect new contributions only toward safety. Existing holdings unchanged.", targets: { VWRA: 55, QQQM: 20, SMH: 10, A35: 15 } },
  { key: "III", label: "Phase III — Locking in gains", range: "SGD 96,000–114,000",  selling: true,  body: "Start gradually moving money to safety. Once per quarter (on your monthly window), sell a small slice of QQQM and VWRA and put the proceeds into A35. Goal: shift from 90% stocks to roughly 80% stocks. Don't touch SMH — it will be liquidated last when you buy the property.", targets: { VWRA: 45, QQQM: 20, SMH: 15, A35: 25 } },
