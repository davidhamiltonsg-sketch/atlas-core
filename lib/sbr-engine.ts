...
export interface SbrEngineOpts {
  drawdownPct?: number
  // Set by callers that persist pause state: once the combined ceiling has been breached,
  // the pause holds until the combined allocation falls below the resume level (hysteresis).
...
export function sbrRoute(positions: SbrPosition[], totalValue: number, opts: SbrEngineOpts = {}, c: Constitution = SILICON_BRICK_ROAD): SbrBranch {
  return route(positions,totalValue,c,opts)
}
...
function result(branch: SbrBranch, c: Constitution, positions: SbrPosition[]): { move: NextMove; selected: string | null } {
  ...
  if(branch.tag==="combined_hard") { const destination=eligibleDestination(positions,new Set(combined?.tickers??[])); return { selected:destination?.ticker??null, move:{severity:"critical",ticker:destination?.ticker??core.ticker,action:"Pause growth satellites",what:`${combined?.label??"The growth satellites"} total ${branch.combined.toFixed(1)}%, above the ${combined?.hard}% ceiling. ${destination?`Route new cash to ${destination.ticker}.`:"Bank new cash pending review."}`,why:`Overlapping growth exposure is governed as one risk; the pause holds until the combined allocation falls below the ${combined?.resume}% resume level.`,when:`Until the combined allocation is below ${combined?.resume}%.`,color:destination?.color??core.color} } }
  if(branch.tag==="combined_watch") { const destination=eligibleDestination(positions,new Set()); return { selected:destination?.ticker??null, move:{severity:"medium",ticker:destination?.ticker??core.ticker,action:destination?`Route cash to ${destination.ticker}`:"Bank this contribution",what:`${combined?.label??"The growth satellites"} total ${branch.combined.toFixed(1)}%, above the ${combined?.warning}% watch level. Contributions continue as normal${destination?` — new cash goes to ${destination.ticker}`:""}.`,why:`The watch level is an early warning only; a pause begins above ${combined?.hard}%.`,when:"At the next permitted dealing window.",color:destination?.color??core.color} } }
...
export function computeSbrNextMove(positions:SbrPosition[],totalValue:number,opts:SbrEngineOpts={},c:Constitution=SILICON_BRICK_ROAD):NextMove { return result(route(positions,totalValue,c,opts),c,positions).move }

export function computeSbrDca(positions:SbrPosition[],monthly:number,opts:SbrEngineOpts={},c:Constitution=SILICON_BRICK_ROAD):DcaPlan {
  const {selected,move}=result(route(positions,positions.reduce((s,p)=>s+p.value,0),c,opts),c,positions)
