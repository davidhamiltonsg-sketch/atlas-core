// ─────────────────────────────────────────────────────────────────────────────
// Look-through live refresh — fetches real fund compositions and upserts them
// into the EtfLookThrough table, which computeLookThrough / computeSbrLookThrough
// prefer over the static fact-sheet baselines in lib/look-through.ts.
//
// Called from two places:
//   • app/reports/actions.ts  — the manual "Refresh look-through" button
//   • app/api/cron/daily      — automatic daily refresh, so §4 concentration
//     estimates track the funds' real rebalances instead of hand-typed tables
//
// Only GOVERNED funds are refreshed (Atlas: VWRA/EQAC/SMH/BTC/DBMFE; SBR adds
// A35). Retired tickers (IMID, IWQU, IB01…) are deliberately absent: their DB
// rows go stale and the freshness logic correctly ignores funds nobody holds.
// ─────────────────────────────────────────────────────────────────────────────
...
const SYMBOL_TO_COMPANY: Record<string, string> = {
  ...
  AVGO:     "Broadcom",
  TSM:      "TSMC",
  "2330.TW":"TSMC",   // Taiwan Stock Exchange listing
}
...
// ── Sources: governed funds only ─────────────────────────────────────────────
// Presets carry funds whose mandate is fixed (Bitcoin, managed futures, the SGD
// bond anchor) — a live holdings feed adds nothing but a failure mode there.
const SOURCES = [
  ...
  { ticker: "EQAC", yahoo: "EQAC.L", source: "https://www.invesco.com/uk/en/financial-products/etfs/invesco-eqqq-nasdaq-100-ucits-etf-acc.html", preset: null },
  { ticker: "SMH", yahoo: "SMH.L", source: "https://www.vaneck.com/uk/en/semiconductor-etf", preset: null },
  ...
  // SBR-only SGD bond anchor. iBoxx ABF Singapore Bond Index: SGD government and
  // quasi-government bonds — Singapore is a developed market, so intlDev 100.
  { ticker: "A35", yahoo: null, source: "https://www.nikkoam.com.sg/etf/abf-singapore-bond", preset: "sgd_bonds" },
] as const

export interface LookThroughRefreshResult {
  updated: string[]
  errors: string[]
}

/** Fetch live compositions for every governed fund and upsert EtfLookThrough rows.
 *  No auth here — callers gate access (server action checks the session; the cron
 *  route checks CRON_SECRET). */
export async function refreshEtfLookThrough(): Promise<LookThroughRefreshResult> {
  const updated: string[] = []
  const errors:  string[] = []

  for (const row of SOURCES) {
    const { ticker, yahoo, source, preset } = row
    try {
      let data: EtfData
      ...
      } else if (preset === "sgd_bonds") {
        data = {
          companyWeights: {},
          sectorWeights: { semiconductor: 0, digital: 0, us: 0, ai: 0 },
          geoWeights: { us: 0, intlDev: 100, emerging: 0, crypto: 0 },
        }
      } else {
