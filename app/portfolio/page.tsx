...
export default async function Portfolio() {
  ...
  // Server-rendered freshness is intentionally evaluated at request time -- this is an async
  // Server Component, not a client render subject to React's memoization/concurrency purity rules.
  // eslint-disable-next-line react-hooks/purity
  const daysSinceUpdate = latestDate ? Math.floor((Date.now() - latestDate.getTime()) / 86_400_000) : null
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
      ...
      <div className="space-y-4">
        ...
        <div id="holdings" className="rounded-xl border border-border bg-card overflow-hidden">
          ...
          <HoldingsTableInteractive
            hasBalance={hasBalance}
            sleeveActual={sleeveActual}
            sleeveGroups={sleeveGroups.map((g) => ({
              key: g.key,
              label: g.label,
              target: g.target,
