...
export default async function Reports(){const session=await getSession();if(!session)redirect("/login");const active=await activePortfolioContext(session);if(active.constitutionId==="silicon-brick-road")return <SbrReportPage userId={active.owner.id} userName={session.name} isAdmin={session.role==="admin"}/>;const d=await getAtlasReportData(active.owner.id,"monthly");const ytd=await getYtdIncomeAndCosts(active.owner.id);return <Shell title="Look-through" subtitle="Atlas Core · every headline reconciled to its fund contributions" userName={session.name} isAdmin={session.role==="admin"} constitutionId="atlas-core"><div className="report-deck">
 ...
 {ytd&&<section className="rounded-xl border border-border bg-card overflow-hidden"><div className="px-5 py-4 border-b border-border"><h2 className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Dividends &amp; costs — year to date</h2></div><div className="p-5 grid grid-cols-3 gap-6"><div><p className="text-xs text-muted-foreground mb-1">Dividends received</p><p className="text-xl font-black tabular-nums text-green-500">{formatCurrency(ytd.dividends,"SGD")}</p></div><div><p className="text-xs text-muted-foreground mb-1">Fees &amp; charges</p><p className={`text-xl font-black tabular-nums ${ytd.fees<0?"text-red-500":"text-foreground"}`}>{formatCurrency(ytd.fees,"SGD")}</p></div><div><p className="text-xs text-muted-foreground mb-1">Interest</p><p className={`text-xl font-black tabular-nums ${ytd.interest<0?"text-red-500":"text-foreground"}`}>{formatCurrency(ytd.interest,"SGD")}</p></div></div><div className="px-5 pb-4"><p className="text-[11px] text-muted-foreground">From imported IBKR activity since 1 January. Figures update after each activity sync.</p></div></section>}
