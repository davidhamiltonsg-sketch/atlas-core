...
eq("Atlas approved tickers", ATLAS_SPEC.funds.map(f => f.ticker), ["IMID", "EQAC", "SMH", "IWQU", "BTC"])
eq("SBR approved tickers", SBR_SPEC.funds.map(f => f.ticker), ["IMID", "EQAC", "SMH", "IB01"])
eq("UCITS identities", ATLAS_SPEC.funds.filter(f => f.ticker !== "BTC").map(f => f.isin), ["IE00B3YLTY66", "IE00BFZXGZ54", "IE00BMC38736", "IE00BP3QZ601"])
...
d = decidePortfolio(ATLAS_CORE, [p("IMID", 52), p("EQAC", 10), p("SMH", 4), p("IWQU", 29), p("BTC", 5)])
eq("Atlas target state remains contribution-first", [d.state, d.move.severity, d.move.ticker], ["invested", "none", "IMID"])

d = decidePortfolio(ATLAS_CORE, [p("IMID", 51), p("EQAC", 16), p("SMH", 4), p("IWQU", 25), p("BTC", 4)])
